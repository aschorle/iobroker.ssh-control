...
class SSHClient {
	...
	connect() {
		...
		this.connecting = new Promise((resolve, reject) => {
			...
			const privateKeyExists = privateKeyPath ? fs.existsSync(privateKeyPath) : false;
			const privateKeySize = privateKeyExists ? getFileSize(privateKeyPath) : undefined;

			this.logDebug(
				`SSHClient privateKeyPath=${privateKeyPath}, privateKeyExists=${privateKeyExists}, privateKeySize=${privateKeySize ?? ''}`,
			);
			this.logDebug(
				`SSH connect: host=${this.options.host}, port=${this.options.port}, username=${this.options.username}, privateKeyPath=${privateKeyPath}, privateKeyRead=${Boolean(privateKey)}`,
			);
...
/**
 * Reads a file size for debug logging.
 *
 * @param {string} filePath - File path.
 * @returns {number | undefined} File size.
 */
