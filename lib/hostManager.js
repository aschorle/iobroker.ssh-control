...
class HostManager {
	...
	normalizeHost(host, index) {
		...
		const normalizedHost = {
			...
			displayOnCommand: host.displayOnCommand || '',
			displayOffCommand: host.displayOffCommand || '',
		};

		if (!normalizedHost.host || !normalizedHost.username) {
	...
	async updateHostInfo(host) {
		try {
			const result = await this.executeSshCommand(host, HOSTNAME_COMMAND);
	...
	async runCommand(host, command) {
		const result = await this.executeSshCommand(host, command);
	...
	/**
	 * Executes the configured display command for one host.
	 *
	 * @param {string} displayStateId - Display state ID.
	 * @param {boolean} enabled - Whether display should be enabled.
	 */
	async executeDisplayAction(displayStateId, enabled) {
		const host = this.findHostByDisplayStateId(displayStateId);

		if (!host) {
			this.adapter.log.warn(`Ignoring display action for unknown SSH host state "${displayStateId}"`);
			return;
		}

		const command = enabled ? host.displayOnCommand.trim() : host.displayOffCommand.trim();

		if (!command) {
			this.adapter.log.warn(`Ignoring empty display command for host "${host.id}"`);
			return;
		}

		try {
			await this.executeSshCommand(host, command);
		} catch (error) {
			this.adapter.log.error(`Could not execute display command for host "${host.id}": ${error.message}`);
		}
	}

	/**
	 * Executes one SSH command.
	 *
	 * @param {{
	 *   id: string,
	 *   stateId: string,
	...
	async executeSshCommand(host, command) {
		this.adapter.log.info(`Creating SSHClient for host id=${host.id}, privateKeyPath=${host.privateKeyPath || ''}`);
	...
	/**
	 * Finds a host by its display state ID.
	 *
	 * @param {string} displayStateId - Display state ID.
	 * @returns {{
	 *   id: string,
	 ...
	 * } | undefined} Matching host.
	 */
	findHostByDisplayStateId(displayStateId) {
		return this.hosts.find(host => displayStateId === `${this.adapter.namespace}.${host.stateId}.actions.display`);
	}
}
